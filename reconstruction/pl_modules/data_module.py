...
from reconstruction.data import CombinedSliceDataset, SliceDataset, VolumeSampler
...
class MriDataModule(pl.LightningDataModule):
    """
    Data module class for the MRI datset used in this project.

    This class handles configurations for training on MRI data. It is set
    ...
    def _create_data_loader(
        ...
        if is_train and self.combine_train_val:
            data_paths = [
                self.data_path / f"train",
                self.data_path / f"valid",
            ]
    ...
    def prepare_data(self):
        ...
        if self.use_dataset_cache_file:
            if self.test_path is not None:
                ...
                test_path = self.data_path / f"test"
            data_paths = [
                self.data_path / f"train",
                self.data_path / f"valid",
                test_path,
    ...
    def val_dataloader(self):
        return self._create_data_loader(
            self.val_transform, data_partition="valid", sample_rate=1.0
        )
    ...
    def add_data_specific_args(parent_parser):  # pragma: no-cover
        ...
        parser.add_argument(
            ...
            default=None,
            type=Path,
            help="Path to data root, expects subdirectories train/valid/test",
        )
        parser.add_argument(
            "--test_path",
            default=None,
