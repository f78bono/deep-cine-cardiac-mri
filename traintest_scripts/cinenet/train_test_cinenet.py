...
from reconstruction.data.transforms import CineNetDataTransform
from reconstruction.pl_modules import MriDataModule, CineNetModule
...
def train_test_main(args, save_path):
    ...
    train_transform = CineNetDataTransform(mask_func=mask, use_seed=False)
    val_transform = CineNetDataTransform(mask_func=mask, use_seed=False)
    test_transform = CineNetDataTransform(mask_func=mask, use_seed=False)
    ...
    if args.load_model:
        ...
        if ckpt_list:
            ...
            model = CineNetModule.load_from_checkpoint(last_ckpt_path)
        else:
    ...
    if args.mode == "train":
        ...
        if args.save_checkpoint:
            trainer.save_checkpoint(args.default_root_dir / f"checkpoints/cinenet.ckpt")
            print(f"Saving checkpoint in cinenet_{args.dynamic_type}_acc{args.accelerations[0]}_ckpt")
    ...
    if (args.mode == "test" and args.inference):
        ...
        dataloader = torch.utils.data.DataLoader(inference_dataset, num_workers=4)
        inf_transform = InferenceTransform(model, 'cinenet', save_path)
        ...
        for batch in dataloader:
            with torch.no_grad():
                masked_kspace, mask, sens_maps, target, fname, _, _, _ = batch
                time_for_inference += inf_transform(masked_kspace, mask, target, fname, sens_maps)
...
def build_args():
    ...
    default_root_dir = fetch_dir("log_path", path_config) / "cinenet/cinenet_logs"
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        ...
        filename=f"cinenet_{args.dynamic_type}_acc{args.accelerations[0]}_ckpt",
        verbose=True,
