...
from reconstruction.data.transforms import XPDNetDataTransform
from reconstruction.pl_modules import MriDataModule, XPDNetModule
...
def train_test_main(args, save_path):
    ...
    train_transform = XPDNetDataTransform(mask_func=mask, use_seed=False)
    val_transform = XPDNetDataTransform(mask_func=mask, use_seed=False)
    test_transform = XPDNetDataTransform(mask_func=mask, use_seed=False)
    ...
    if args.load_model:
        ...
        if ckpt_list:
            ...
            model = XPDNetModule.load_from_checkpoint(last_ckpt_path)
        else:
        ...
        model = XPDNetModule(
            ...
            crnn_chans=args.crnn_chans,
            n_scales=args.n_scales,
            n_filters_per_scale=args.n_filters_per_scale,
            n_convs_per_scale=args.n_convs_per_scale,
            n_first_convs=args.n_first_convs,
            first_conv_n_filters=args.first_conv_n_filters,
            res=args.res,
            primal_only=args.primal_only,
            n_primal=args.n_primal,
            n_dual=args.n_dual,
    ...
    if args.mode == "train":
        ...
        if args.save_checkpoint:
            trainer.save_checkpoint(args.default_root_dir / f"checkpoints/xpdnet.ckpt")
            print(f"Saving checkpoint in xpdnet_{args.dynamic_type}_acc{args.accelerations[0]}_ckpt")
    ...
    if (args.mode == "test" and args.inference):
        ...
        inf_transform = InferenceTransform(model, 'xpdnet', save_path)
...
def build_args():
    ...
    default_root_dir = fetch_dir("log_path", path_config) / "xpdnet/xpdnet_logs"
    ...
    # Model config
    parser = XPDNetModule.add_model_specific_args(parser)
    parser.set_defaults(
        num_cascades=9,
        sens_chans=8,
        sens_pools=3,
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        ...
        filename=f"xpdnet_{args.dynamic_type}_acc{args.accelerations[0]}_ckpt",
        verbose=True,
